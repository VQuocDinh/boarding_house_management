...
import FormAddSpendBill from './Components/FormAddSpendBill';
...
import ShowDetailSpend from './Components/ShowDetailSpend';
import RemoveRedEyeIcon from '@mui/icons-material/RemoveRedEye';
function SpendPage(props) {
    ...
    const [dataSpendBill,setDataSpendBill] = useState([]);
    const date = new Date();
    ...
    useEffect(() => {
        (async () => {
            try {
                const dataSpendBill = await normalApi.getSpendBill(filter)
                setDataSpendBill(dataSpendBill.data)
                numberItem.current = (dataSpendBill.count)
                console.log(dataSpendBill);
    ...
    const handleDeleteSpendBill = async () => {
        const value = {
            id : globalValue.current
        }
        try {
            const data = await adminApi.deleteSpendBill(value);
            setFilter(x => ({...x}))
    ...
    const handleToExcel =async () => {
        try {
            await toExcel.exportExcel(dataSpendBill,"Danh phiếu chi tháng","ListSpendBill")
        } catch (error) {
    ...
    return (
        <div className='spendPage-container'>
                ...
                <div className='header'>
                    ...
                    <div className='buttons'>
                        <div className='button' onClick={() => handleShowForm(1)}>
                            <Button variant="contained" endIcon={<AddIcon />}>Thêm phiếu chi</Button>
                        </div>
            ...
            >
                <DialogContent>
                    {statusForm == 1 && <FormAddSpendBill dataHouse={arrayHouseData.current} onCloseDiagram={handleClose} reLoad={setFilter}/>}
                    {statusForm == 2 && <FormAddSpendBill dataHouse={arrayHouseData.current} onCloseDiagram={handleClose} reLoad={setFilter} dataApi={globalValue.current} status="update"/>}
                    {statusForm == 3 && <FormDelete header="Xóa phiếu chi" content="Khi xóa sẽ không thể khôi phục, dữ liệu khi tính lại lời lỗ phòng tháng sẽ thay đổi." onCloseDiagram={handleClose}  onSubmit={handleDeleteSpendBill}/>}
                    {statusForm == 4 && <ShowDetailSpend data={globalValue.current} onCloseDiagram={handleClose}/>}
                </DialogContent>
            </Dialog>
        </div>
    );
