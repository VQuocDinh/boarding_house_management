...
function FormAddWater({dataHouse=[],onCloseDiagram,dataApi = {},status,reLoad}) {
    const phoneRegExp = /^\d+$/
    ...
    const handleSubmitForm = async (value) => {
        ...
        try {
            ...
            if(status == "update")
            {
                ...
                data = await normalApi.updateWaterBill(value)
            }
            else{
                data = await normalApi.addWaterBill(value);
            }
            reLoad(x => ({...x}))
            onCloseDiagram()
    ...
    return (
        <div className='formAddElectric-container'>
            <div className='formAddElectric-content'>
                <p className='header'>Hóa đơn nước tháng</p>
                <form onSubmit={form.handleSubmit(handleSubmitForm)}>
                        ...
                        <div className='input'>
                            <DateField name='from_time' label="Ngày bắt đầu" form={form}/>
                        </div>
                        <div className='input'>
                            <DateField name='to_time' label="Ngày kết thúc" form={form}/>
                        </div>
                        ...
                        </div>
                        {
                            status == "update" && (
                                <div className='input update-note-form'>
                                    ...
                                    Khi thay đổi phiếu nước, bạn cần tính toán lại hóa đơn tháng liên quan đến nước điện đó!`}</p>
                                </div>
                            )
                        }
