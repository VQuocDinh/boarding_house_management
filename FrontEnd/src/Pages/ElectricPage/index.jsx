...
import FormAddElectric from './Components/FormAddBillElec';
...
function ElectricPage(props) {
    ...
    useEffect(() => {
        (async () => {
            try {
                const dataBill = await normalApi.getBillElectric(filter);
    ...
    const handleToExcel =async () => {
        try {
            await toExcel.exportExcel(dataBill,"Danh sách điện tháng","ListElectricBill")
        } catch (error) {
    ...
    return (
        <div className='electricPage-container'>
            ...
            >
                <DialogContent>
                    {statusForm == 1 && <FormAddElectric dataHouse={arrayHouseData.current} onCloseDiagram={handleClose} reLoad={setFilter}/>}
                    {statusForm == 2 && <FormAddElectric dataHouse={arrayHouseData.current} onCloseDiagram={handleClose} dataApi={globalValue.current} status="update" reLoad={setFilter}/>}
                </DialogContent>
            </Dialog>
        </div>
    );
